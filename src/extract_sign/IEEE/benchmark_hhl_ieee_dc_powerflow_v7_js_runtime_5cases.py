...
network. Five cases are included by default:

    case4gs -> 4 buses  -> 3x3
    case5   -> 5 buses  -> 4x4
    case6ww -> 6 buses  -> 5x5
    case9   -> 9 buses  -> 8x8
    case14  -> 14 buses -> 13x13 (padded to 16x16 for HHL)

The added case14 is the IEEE 14-bus system. It raises the target-register size
from three to four qubits and increases full tomography from 27 to 81
measurement bases, so its runtime can be substantially larger.
...
Recommended usage:
  ...
  python benchmark_hhl_ieee_dc_powerflow_v7_js_runtime_5cases.py --method parity --phase-qubits 4 --shots 20000

  # 2) Run tomography, reusing exactly the same saved systems
  python benchmark_hhl_ieee_dc_powerflow_v7_js_runtime_5cases.py --method tomography --phase-qubits 4 --tomo-shots 20000

  # Or run both methods in one command
  python benchmark_hhl_ieee_dc_powerflow_v7_js_runtime_5cases.py --method both --phase-qubits 4 --shots 20000 --tomo-shots 20000

  # Replot existing CSV data only
  python benchmark_hhl_ieee_dc_powerflow_v7_js_runtime_5cases.py --plot-only

Default outputs:
  benchmark_outputs_ieee_dc_js_runtime_5cases/
...
Notes:
  ...
  - HHL requires dimension 2^n. The existing solver modules are expected to
    auto-pad 3x3, 5x5, and 13x13 systems to 4x4, 8x8, and 16x16.
  - Runtime is plotted separately and is also included in the dashboard.
...
SCRIPT_VERSION = "2026-07-25-dc-v7-js-runtime-5cases"

# =============================================================================
# Output and case configuration
# =============================================================================
DEFAULT_OUTPUT_DIR = Path("benchmark_outputs_ieee_dc_js_runtime_5cases")
DEFAULT_SYSTEM_DIR_NAME = "systems"
DEFAULT_SOLUTION_DIR_NAME = "solutions"
DEFAULT_CSV_NAME = "hhl_ieee_dc_js_runtime_5cases_results.csv"

SUPPORTED_CASES = ("case4gs", "case5", "case6ww", "case9", "case14")
...
def _create_individual_similarity_plot(
    ...
    ax.set_xlabel("System size (bus / matrix size)")
...
def _format_runtime_seconds(value: float) -> str:
    """Compact human-readable runtime label."""
    value = float(value)
    if value < 60.0:
        return f"{value:.1f} s"
    minutes = value / 60.0
...
def _plot_runtime_axis(
    ...
    """Plot measured HHL module runtime for both recovery methods."""
    x_positions = np.arange(len(case_names))
    ...
    for method in ("parity_check", "tomography"):
        ...
        all_positive.extend(value for value in values if np.isfinite(value) and value > 0.0)
        ...
        for x, value in zip(x_positions, values):
            if np.isfinite(value):
                # Nếu điểm ở thấp gần trục x thì đẩy label lên trên để tránh đè lên tick labels
                if value < 60:
                    offset_y = 8
                else:
                    offset_y = 8 if method == "tomography" else -18

                ax.annotate(
                    _format_runtime_seconds(value),
                    (x, value),
                    xytext=(0, offset_y),
                    textcoords="offset points",
                    ha="center",
    ...
    ax.set_xticks(x_positions)
    ax.set_xticklabels(_case_tick_labels(rows, case_names), fontsize=8.3)
    ...
    if runtime_scale == "log" and all_positive:
        ax.set_yscale("log")
        ax.set_ylabel("Runtime")
    ...
    finite_values = [
        float(by_method_case[(method, case_name)]["runtime_seconds"])
        for method in ("parity_check", "tomography")
        for case_name in case_names
        if (method, case_name) in by_method_case
    ]
    if finite_values and runtime_scale == "linear":
        ax.set_ylim(0.0, max(finite_values) * 1.08)


def _create_individual_runtime_plot(
    rows: list[dict[str, Any]],
    case_names: list[str],
    by_method_case: dict[tuple[str, str], dict[str, Any]],
    output_dir: Path,
    ...
    _plot_runtime_axis(ax, rows, case_names, by_method_case, runtime_scale)
    ax.set_xlabel("System size (bus / matrix size)")
    fig.tight_layout()
...
def create_js_dashboard(
    ...
    fig = plt.figure(figsize=(22.5, 12.6))
    fig.tight_layout()
    ...
    grid = GridSpec(2, 18, figure=fig, height_ratios=[1.15, 0.95])
    ...
    ax_summary = fig.add_subplot(grid[0, 14:18])
    ax_summary.axis("off")
    ...
    ax_amp_line = fig.add_subplot(grid[1, 0:6])
    _plot_similarity_axis(
        ax_amp_line,
        "js_similarity_amplitude_percent",
        "JS similarity — amplitude distribution",
        rows,
        case_names,
        by_method_case,
    )

    ax_signed_line = fig.add_subplot(grid[1, 6:12])
    _plot_similarity_axis(
        ax_signed_line,
        "js_similarity_signed_percent",
        "JS similarity — signed distribution",
        rows,
        case_names,
        by_method_case,
    )

    ax_runtime = fig.add_subplot(grid[1, 12:18])
    _plot_runtime_axis(
        ax_runtime,
        rows,
        case_names,
    ...
    fig.text(
        ...
        "Runtime reports only the HHL recovery module; use relative residual for equation-level validation.",
        ha="center",
        fontsize=9.8,
    )
...
def plot_results(
    csv_path: Path,
    output_dir: Path,
    dashboard_case: str = "case14",
    runtime_scale: str = "log",
    latest_only: bool = True,
) -> tuple[Path, Path, Path, Path]:
    """Create JS metric charts, a runtime chart, and one combined dashboard."""
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    case_names = ["case4gs", "case5", "case6ww", "case9", "case14"]
    by_method_case = {(r["method"], r["case_name"]): r for r in rows}
    ...
    runtime_path = _create_individual_runtime_plot(
        rows,
        case_names,
        by_method_case,
        output_dir,
    ...
    dashboard_path = create_js_dashboard(
        rows,
        output_dir,
        dashboard_case,
        runtime_scale,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Reject a reduced system larger than this value; case14 requires at least 13.",
    )
    ...
    parser.add_argument(
        ...
        choices=["linear", "log"],
        default="linear",
        help="Vertical scale for runtime plots. ",
    )
    parser.add_argument(
        "--prepare-only",
        action="store_true",
...
def main() -> None:
    ...
    if not args.no_plot:
        try:
            amplitude_js_plot, signed_js_plot, runtime_plot, dashboard_plot = plot_results(
                csv_path,
                output_dir,
