...
network. The first four reduced matrices are below 10. This extended version also adds three larger standard cases:

    case4gs -> 4 buses -> 3x3
    case5   -> 5 buses -> 4x4
    case6ww -> 6 buses -> 5x5
    case9            -> 9 buses  -> 8x8
    case14           -> 14 buses -> 13x13
    case24_ieee_rts  -> 24 buses -> 23x23
    case30           -> 30 buses -> 29x29
...
Recommended usage:
  ...
  python benchmark_hhl_ieee_dc_powerflow_v7_7cases_js_runtime.py --method parity --phase-qubits 4 --shots 20000

  # 2) Run tomography, reusing exactly the same saved systems
  python benchmark_hhl_ieee_dc_powerflow_v7_7cases_js_runtime.py --method tomography --phase-qubits 4 --tomo-shots 20000

  # Or run both methods in one command
  python benchmark_hhl_ieee_dc_powerflow_v7_7cases_js_runtime.py --method both --phase-qubits 4 --shots 20000 --tomo-shots 20000

  # Replot existing CSV data only
  python benchmark_hhl_ieee_dc_powerflow_v7_7cases_js_runtime.py --plot-only

Default outputs:
  benchmark_outputs_ieee_dc_js_runtime_7cases/

Notes:
  - The input network data are not random. They come from pandapower's standard
    MATPOWER/PYPOWER-derived test cases.
  - HHL requires dimension 2^n. The solver modules are expected to auto-pad
    3, 5, 13, 23, and 29 dimensional systems to 4, 8, 16, 32, and 32.
  ...
  - Runtime is saved in CSV and plotted on a logarithmic axis because tomography
    cost grows rapidly with the number of target qubits.
"""
...
SCRIPT_VERSION = "2026-07-25-dc-v7-7cases-js-runtime"

# =============================================================================
# Output and case configuration
# =============================================================================
DEFAULT_OUTPUT_DIR = Path("benchmark_outputs_ieee_dc_js_runtime_7cases")
DEFAULT_SYSTEM_DIR_NAME = "systems"
DEFAULT_SOLUTION_DIR_NAME = "solutions"
DEFAULT_CSV_NAME = "hhl_ieee_dc_js_runtime_7cases_results.csv"
...
def _create_individual_similarity_plot(
    ...
    fig, ax = plt.subplots(figsize=(14.0, 6.3))
...
def _format_runtime(seconds: float) -> str:
    """Compact runtime label for plot annotations."""
    if seconds < 1.0:
...
def _plot_runtime_axis(
    ...
    """Plot HHL solver runtime for both recovery methods.

    A logarithmic y-axis is used because tomography runtime may grow by orders
    of magnitude for case14, case24_ieee_rts, and case30.
    """
    x_positions = np.arange(len(case_names))
    ...
    for method in ("parity_check", "tomography"):
        values = [
            np.nan if (method, case_name) not in by_method_case
            else float(by_method_case[(method, case_name)]["runtime_seconds"])
            for case_name in case_names
        ]
        ...
        for x, value in zip(x_positions, values):
            if np.isfinite(value) and value > 0.0:
                ...
                ax.annotate(
                    _format_runtime(float(value)),
                    (x, value),
                    xytext=(0, 8 if method == "tomography" else -18),
                    textcoords="offset points",
                    ha="center",
                    fontsize=8.5,
                )
    ...
    ax.set_xticks(x_positions)
    ax.set_xticklabels(_case_tick_labels(rows, case_names), fontsize=8.4)
    ax.set_yscale("log")
    ax.set_ylabel("HHL solver runtime (seconds, log scale)")
    ax.set_title("HHL solution-recovery runtime", fontweight="bold")
    ax.grid(True, which="both", alpha=0.25)
    ax.legend(loc="best")


def _create_runtime_plot(
    rows: list[dict[str, Any]],
    case_names: list[str],
    by_method_case: dict[tuple[str, str], dict[str, Any]],
    output_dir: Path,
    ...
    fig, ax = plt.subplots(figsize=(14.0, 6.3))
    _plot_runtime_axis(ax, rows, case_names, by_method_case)
    ax.set_xlabel("Standard power-system case and reduced DC matrix size")
    fig.tight_layout()
    path = output_dir / "dc_hhl_runtime_comparison.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path
...
def create_js_dashboard(
    ...
    """Create a publication-style JS dashboard with an additional runtime panel."""
    import matplotlib.pyplot as plt
    ...
    if not case_names:
        raise RuntimeError("No supported cases are available for the dashboard.")
    ...
    if not methods:
        raise RuntimeError(
            f"No valid solution data found for dashboard case {dashboard_case}."
        )
    ...
    fig = plt.figure(figsize=(21.5, 16.8))
    fig.subplots_adjust(
        left=0.035,
        right=0.985,
        top=0.91,
    ...
    fig.text(
        0.5,
        0.942,
        "Seven DC power-flow cases — Parity check versus Tomography",
        ha="center",
        fontsize=13.5,
    )
    ...
    formula_text = (
        ...
        "Runtime is measured only around\n"
        "the HHL recovery solver."
    )
    ax_formula.text(
        0.02,
        0.98,
    ...
    ax_amp.set_xticklabels(
        x_labels,
        rotation=45 if n > 14 else 0,
        ha="right" if n > 14 else "center",
        fontsize=7.5 if n > 14 else 8.5,
    )
    ...
    ax_amp.legend(fontsize=8.5, loc="upper right")
    ...
    ax_signed.set_xticklabels(
        signed_labels,
        rotation=65 if n > 14 else 50,
        ha="right",
        fontsize=6.2 if n > 14 else 7.3,
    )
    ...
    ax_signed.legend(fontsize=8.2, loc="upper right")
    ...
    summary_lines = [
        ...
        f"Matrix: {row_any['matrix_size']}×{row_any['matrix_size']}",
        f"Target qubits: {row_any['target_qubits']}",
    ...
    ax_runtime = fig.add_subplot(grid[2, 0:16])
    _plot_runtime_axis(ax_runtime, rows, case_names, by_method_case)
    ax_runtime.set_xlabel("Power-system case and reduced DC matrix size")

    fig.text(
        0.5,
        0.025,
        "Interpretation: high amplitude similarity but low signed similarity indicates sign-recovery errors. "
        "Runtime uses a logarithmic scale because tomography cost rises rapidly with target-qubit count.",
        ha="center",
        fontsize=9.6,
    )

    path = output_dir / "dc_js_relative_entropy_runtime_dashboard.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path
...
def plot_results(
    ...
    """Create JS similarity charts, runtime chart, and one combined dashboard."""
    ...
    runtime_path = _create_runtime_plot(
        rows,
        case_names,
        by_method_case,
        output_dir,
    ...
    return amplitude_path, signed_path, runtime_path, dashboard_path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "systems from seven standard MATPOWER/PYPOWER-derived cases."
        )
    )
    parser.add_argument(
        "--method",
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Reject a reduced system larger than this value (case30 requires at least 29).",
    )
...
def main() -> None:
    ...
    if not args.no_plot:
        try:
            ...
            print(f"[PLOT] Runtime comparison:      {runtime_plot}")
            print(f"[PLOT] JS/runtime dashboard:    {dashboard_plot}")
        except Exception as exc:
